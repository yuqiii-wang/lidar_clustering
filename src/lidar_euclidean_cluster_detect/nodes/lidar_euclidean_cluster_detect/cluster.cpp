                       ...
                       int in_g, int in_b, std::string in_label, bool in_estimate_pose)
{
...

  {
    ...
    // cv::Mat localPtImg = cv::Mat::zeros(1000, 1000, cv::CV_8UC1);
    // for (int i = 0; i < current_cluster->points.size(); i++)
    // {
    //   localPtImg.at<uchar>(current_cluster->points[i].x, current_cluster->points[i].y, 0) = 1;
    // }
    // std::vector<std::vector<cv::Point2f>> contours;
    // cv::findContours( localPtImg, contours, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE );
    // std::cout << "finished " << contours.size() << std::endl;
