                                        ...
                                        double in_max_cluster_distance = 0.5)
{
  ...
  //   // for large clusters, segment them again 
  //   if (it->indices.size() > 300)
  //   {

  std::vector<pcl::PointIndices> clusterLocal_indices;
  customClusteringExt::clusterByNormalAndDist(
    cloud_2d, clusterLocal_indices);
  
  for (auto clusterLocal_index = clusterLocal_indices.begin();
      clusterLocal_index != clusterLocal_indices.end(); 
      clusterLocal_index++)
  {
    ClusterPtr cluster(new Cluster());
    cluster->SetCloud(in_cloud_ptr, clusterLocal_index->indices, _velodyne_header, k, 
                    (int) _colors[k].val[0],
                    (int) _colors[k].val[1],
                    (int) _colors[k].val[2], 
                    "", _pose_estimation);
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  if (!_using_sensor_cloud)
  {
    ...
    // just to clip point cloud by defined height
